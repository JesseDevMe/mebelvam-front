...
import {CustomFilterValue} from "@/entities/Filter";
...
const Filter: FC<FilterProps> = ({ title, slug, values }) => {
    const [isOpen, setIsOpen] = useState(false);
    const customFiltersStore = useCustomFiltersStore();

    const filterRef = useRef<HTMLDivElement>(null);

    useEffect(() => {
        if (customFiltersStore.filters.size === 0)
            setIsOpen(false);
    }, [customFiltersStore.isOpen])

    function handleToggle() {
        setIsOpen(!isOpen);
    }

    function handleCheckChange(isChecked: boolean, valueId: number) {
        if (isChecked) {
            customFiltersStore.deleteValue(slug, valueId);
        } else customFiltersStore.add(slug, valueId);
    }
    ...
    useEffect(() => {
        if (!filterRef.current || !customFiltersStore.isOpen) {
            return;
        }
    ...
    }, [window.innerWidth, filterRef.current, customFiltersStore.isOpen])

    return (
        <div ref={filterRef} className={`py-2.5 px-5 w-full border ${isOpen ? 'border-accent' : 'border-dark'} rounded font-roboto 
        ...
        >
            ...
            >
                ...
                {
                    values.map((value) => {
                        const isChecked = customFiltersStore.filters.get(slug)?.includes(value.id) || false;
                        ...
                        return <li key={value.id}>
                                ...
                                <span className="w-5 h-5 relative border border-dark rounded-[5px]">
                                    <input
                                        ...
                                        checked={isChecked}
                                        onChange={() => handleCheckChange(isChecked, value.id)}
                                    />
