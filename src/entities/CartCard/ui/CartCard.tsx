...
const CartCard: FC<CartCardProps> = ({ id, name, imageUrl, color, size , price, count, variantId, attrId, oldPrice, isModular}) => {
    ...
    return (
        <Link href={`/product/${id}`} className="relative flex flex-col gap-5 md:flex-row py-5 border-b-2 group">
            <div
                onClick={deleteHandler}
                className="absolute right-0 top-4"
            >
                <svg className="hover:scale-110" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
            ...
            <div className="grow md:flex md:flex-col justify-between">
                <h2 className="font-montserrat text-base font-semibold h-[2.9em] line-clamp-2 mr-8 group-hover:text-accent">{name}</h2>
                <div className="mt-7">
